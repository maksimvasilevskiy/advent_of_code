const stringArray =`twovgtprdzcjjzkq3ffsbcblnpq
...
stringArray.split('\n').forEach((str) => {
    ...
    while (!noMoreDigits) {
        ...
        digits.forEach((digit) => {
            if (digitsString[j].indexOf(digit) !== -1) {
                noMoreDigits = false;
                if (digitsString[j].indexOf(digit) < firstIndex || (firstIndex === -1)) {
                    firstIndex = digitsString[j].indexOf(digit);
                    first = j+1;
                }
                if (digitsString[j].indexOf(digit) > lastIndex) {
                    lastIndex = digitsString[j].indexOf(digit);
                    last = j+1;
                }
                let replaceStr = '';
                ...
                digitsString[j] = digitsString[j].replace(digit, replaceStr);
            }
            j++;
        });
    }
